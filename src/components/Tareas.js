...
const Tareas = () => {
    ...
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mt-10">
        <h1 className="text-3xl font-bold mb-6">Tablero de Tareas</h1>
        <div className="mb-6">
            <input
        ...
        <ul className="mt-4">
            {tasks.map(task => (
                <li key={task.id} className="flex items-center justify-between mb-4">
                    <span className={task.completed ? "line-through text-gray-500 text-lg" : "text-black text-lg"}>
                        {task.text}
                    </span>
                    <div>
                        <button
