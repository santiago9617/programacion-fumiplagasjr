...
const Programacion1 = () => {
  ...
  useEffect(() => {
    // Función para mostrar la alerta cuando el usuario intenta salir de la página
    const handleWindowAlert = event => {
      event.preventDefault();
      event.returnValue = ''; // Mensaje personalizado para mostrar al usuario (no funciona en todos los navegadores)
      return ''; // Mensaje personalizado para mostrar al usuario (no funciona en todos los navegadores)
    };

    // Agregar el evento de alerta
    window.addEventListener('beforeunload', handleWindowAlert);

    return () => {
      // Eliminar el evento de alerta al desmontar el componente
      window.removeEventListener('beforeunload', handleWindowAlert);
    };
  }, []);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-8">
        <div className="px-4 py-5 sm:px-6">
          <h3 className="text-lg font-medium leading-6 text-gray-900"><h2>Jhon Mario</h2>Programación de Servicios</h3>
        </div>
